        ...
        <div class="form-group my-3">
            ...
            {% if details.meta.form_elem is defined %}
                ...
                {% if details.meta.form_elem == "numeric" %}
                    <input type="number" min="0" step="0.01" class="form-control" id="{{ field }}" name="{{ field }}" placeholder="{{ details.meta.label }}" v-model="info.form.{{ field }}">
                    <span class="kt-input-icon__icon kt-input-icon__icon--left"><span><i class="la la-user"></i></span>            
                {% endif %}
                {% if details.meta.form_elem == "email" %}
                    <input type="email" class="form-control" id="{{ field }}" name="{{ field }}" placeholder="{{ details.meta.label }}" v-model="info.form.{{ field }}">
                    <span class="kt-input-icon__icon kt-input-icon__icon--left"><span><i class="la la-user"></i></span>            
                {% endif %}
                ...
                {% if details.meta.form_elem == "static_dropdown" %}
                    <?php echo form_dropdown('{{ field }}', Dropdown::get_static('{{ details.meta.static_dropdown }}'), null, 'class="form-control" v-model="info.form.{{ field }}"'); ?>
                    <span class="kt-input-icon__icon kt-input-icon__icon--left"><span><i class="la la-user"></i></span>            
                {% endif %}
            {% else %}            
                <input type="text" class="form-control" id="{{ field }}" name="{{ field }}" placeholder="{{ details.meta.label }}" v-model="info.form.{{ field }}">
                <span class="kt-input-icon__icon kt-input-icon__icon--left"><span><i class="la la-user"></i></span>            
            {% endif %}
            </div>
